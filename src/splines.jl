...
type Spline{T}
	...
	Spline{T}(x::T, params::Vector{Float64}) = begin
...
function splineint{T}(s::Spline{T}, x_out::T)
	...
	return params[base_idx + 1] + params[base_idx + 2]*x_out + 
		params[base_idx + 3]*(x_out^2) + params[base_idx + 4]*(x_out^3)
end
...
function splinefit{T}(x_in::Vector{T}, y_in::Vector{Float64})
	...
	poly_count = points_count - 1
	matrix_n = 4poly_count # the main matrix is a square matrix matrix_n by matrix_n
